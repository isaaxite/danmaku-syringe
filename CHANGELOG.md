...
# bugfix: 播放进度 > 30s 时开始消费弹幕误 2 个相同内容的弹幕池

最大可能是因为 timecount 是使用自增的，当 currentTime > 30s，假设是 45s，则 cnt 是 1，而 timeCount 初始是 -1，需要自增 2 次，即连续插入两次。

另外，再继续跳转到其他的 currentTime，重复的内容会改善。但那是因为弹幕池的数量上限是 2, 但实际弹幕实例被多次新建与销毁，若 cnt 与 timeCount 相差较大的话，次数更加大！

---

实验证实。使用腾讯弹幕源（需要定时请求的类型），视频时间跳转到 2:36 稍前，打印结果：新建与销毁进行了 7 个周期
